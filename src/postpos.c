...
static pcvs_t PcvSat = {0}; /* receiver antenna parameters */
static pcvs_t PcvRcv = {0}; /* satellite antenna parameters */
static obs_t Obss    = {0}; /* observation data */
static nav_t Navs    = {0}; /* navigation data */
static sta_t Stas[MAXRCV]; /* station infomation */
static int Nepoch    = 0;  /* number of observation epochs */
static int IobsRover = 0;  /* current rover observation data index */
static int IobsBase  = 0;  /* current reference observation data index */
static int Revs   = 0;          /* analysis direction (0:forward,1:backward) */
static int Aborts = 0;          /* abort status */
static sol_t *SolForward;       /* forward solutions */
static sol_t *SolBack;          /* backward solutions */
static double *RbaseForward;    /* forward base positions */
static double *RbaseBack;       /* backward base positions */
static int IsolForward    = 0;  /* current forward solutions index */
static int IsolBack       = 0;  /* current backward solutions index */
static char ProcRover[64] = ""; /* rover for current processing */
static char ProcBase[64]  = ""; /* base station for current processing */
...
static int checkbrk(const char *format, ...) {
    ...
    va_end(arg);
    if (*ProcRover && *ProcBase)
        sprintf(p, " (%s-%s)", ProcRover, ProcBase);
    else if (*ProcRover)
        sprintf(p, " (%s)", ProcRover);
    else if (*ProcBase)
        sprintf(p, " (%s)", ProcBase);
    return showmsg(buff);
}
...
static int inputobs(obsd_t *obs, int solq, const prcopt_t *popt) {
    ...
    trace(3, "infunc  : revs=%d iobsu=%d iobsr=%d isbs=%d\n", Revs, IobsRover, IobsBase);

    if (0 <= IobsRover && IobsRover < Obss.n) {
        settime((time = Obss.data[IobsRover].time));
    ...
    if (!Revs) { /* input forward data */
        if ((nu = nextobsf(&Obss, &IobsRover, 1)) <= 0)
            return -1;
        if (popt->intpref) {
            for (; (nr = nextobsf(&Obss, &IobsBase, 2)) > 0; IobsBase += nr)
                if (timediff(Obss.data[IobsBase].time, Obss.data[IobsRover].time) > -DTTOL)
                    break;
        } else {
            for (i = IobsBase; (nr = nextobsf(&Obss, &i, 2)) > 0; IobsBase = i, i += nr)
                if (timediff(Obss.data[i].time, Obss.data[IobsRover].time) > DTTOL)
                    break;
        }
        nr = nextobsf(&Obss, &IobsBase, 2);
        if (nr <= 0) {
            nr = nextobsf(&Obss, &IobsBase, 2);
        }
        for (i = 0; i < nu && n < MAXOBS * 2; i++)
            obs[n++] = Obss.data[IobsRover + i];
        for (i = 0; i < nr && n < MAXOBS * 2; i++)
            obs[n++] = Obss.data[IobsBase + i];
        IobsRover += nu;
    } else { /* input backward data */
        if ((nu = nextobsb(&Obss, &IobsRover, 1)) <= 0)
            return -1;
        if (popt->intpref) {
            for (; (nr = nextobsb(&Obss, &IobsBase, 2)) > 0; IobsBase -= nr)
                if (timediff(Obss.data[IobsBase].time, Obss.data[IobsRover].time) < DTTOL)
                    break;
        } else {
            for (i = IobsBase; (nr = nextobsb(&Obss, &i, 2)) > 0; IobsBase = i, i -= nr)
                if (timediff(Obss.data[i].time, Obss.data[IobsRover].time) < -DTTOL)
                    break;
        }
        nr = nextobsb(&Obss, &IobsBase, 2);
        for (i = 0; i < nu && n < MAXOBS * 2; i++)
            obs[n++] = Obss.data[IobsRover - nu + 1 + i];
        for (i = 0; i < nr && n < MAXOBS * 2; i++)
            obs[n++] = Obss.data[IobsBase - nr + 1 + i];
        IobsRover -= nu;
    }
    return n;
}
...
static void procpos(FILE *fp, const prcopt_t *popt, const solopt_t *sopt, int mode) {
    ...
    while ((nobs = inputobs(obs, rtk.sol.stat, popt)) >= 0) {
        ...
        if (mode == 0) { /* forward/backward */
            ...
            if (IsolForward >= Nepoch)
                return;
            SolForward[IsolForward] = rtk.sol;
            for (i = 0; i < 3; i++)
                RbaseForward[i + IsolForward * 3] = rtk.rb[i];
            IsolForward++;
        } else { /* combined-backward */
            if (IsolBack >= Nepoch)
                return;
            SolBack[IsolBack] = rtk.sol;
            for (i = 0; i < 3; i++)
                RbaseBack[i + IsolBack * 3] = rtk.rb[i];
            IsolBack++;
        }
    }
...
static void combres(FILE *fp, const prcopt_t *popt, const solopt_t *sopt) {
    ...
    trace(3, "combres : isolf=%d isolb=%d\n", IsolForward, IsolBack);

    solstatic = sopt->solstatic && (popt->mode == PMODE_STATIC || popt->mode == PMODE_PPP_STATIC);

    for (i = 0, j = IsolBack - 1; i < IsolForward && j >= 0; i++, j--) {

        if ((tt = timediff(SolForward[i].time, SolBack[j].time)) < -DTTOL) {
            sols = SolForward[i];
            for (k = 0; k < 3; k++)
                rbs[k] = RbaseForward[k + i * 3];
            j++;
        } else if (tt > DTTOL) {
            sols = SolBack[j];
            for (k = 0; k < 3; k++)
                rbs[k] = RbaseBack[k + j * 3];
            i--;
        } else if (SolForward[i].stat < SolBack[j].stat) {
            sols = SolForward[i];
            for (k = 0; k < 3; k++)
                rbs[k] = RbaseForward[k + i * 3];
        } else if (SolForward[i].stat > SolBack[j].stat && SolBack[j].stat != SOLQ_NONE) {// fixed: SYS_NONE case
            sols = SolBack[j];
            for (k = 0; k < 3; k++)
                rbs[k] = RbaseBack[k + j * 3];
        } else {
            ...
            for (k = 0; k < 3; k++) // fixed: ppp combined filter rbs are not calculated 
                rbs[k] = RbaseForward[k + j * 3];

            if ((popt->mode == PMODE_KINEMA || popt->mode == PMODE_MOVEB) && sols.stat == SOLQ_FIX) {

                /* degrade fix to float if validation failed */
                if (!valcomb(SolForward + i, SolBack + j))
                    sols.stat = SOLQ_FLOAT;
            }
            for (k = 0; k < 3; k++) {
                Qf[k + k * 3] = SolForward[i].qr[k];
                Qb[k + k * 3] = SolBack[j].qr[k];
            }
            Qf[1] = Qf[3] = SolForward[i].qr[3];
            Qf[5] = Qf[7] = SolForward[i].qr[4];
            Qf[2] = Qf[6] = SolForward[i].qr[5];
            Qb[1] = Qb[3] = SolBack[j].qr[3];
            Qb[5] = Qb[7] = SolBack[j].qr[4];
            Qb[2] = Qb[6] = SolBack[j].qr[5];

            if (popt->mode == PMODE_MOVEB) {
                for (k = 0; k < 3; k++)
                    rr_f[k] = SolForward[i].rr[k] - RbaseForward[k + i * 3];
                for (k = 0; k < 3; k++)
                    rr_b[k] = SolBack[j].rr[k] - RbaseBack[k + j * 3];
                if (smoother(rr_f, Qf, rr_b, Qb, 3, rr_s, Qs))
                    continue;
                for (k = 0; k < 3; k++)
                    sols.rr[k] = rbs[k] + rr_s[k];
            } else {
                if (smoother(SolForward[i].rr, Qf, SolBack[j].rr, Qb, 3, sols.rr, Qs))
                    continue;
            }
            ...
            if (popt->dynamics) {
                for (k = 0; k < 3; k++) {
                    Qf[k + k * 3] = SolForward[i].qv[k];
                    Qb[k + k * 3] = SolBack[j].qv[k];
                }
                Qf[1] = Qf[3] = SolForward[i].qv[3];
                Qf[5] = Qf[7] = SolForward[i].qv[4];
                Qf[2] = Qf[6] = SolForward[i].qv[5];
                Qb[1] = Qb[3] = SolBack[j].qv[3];
                Qb[5] = Qb[7] = SolBack[j].qv[4];
                Qb[2] = Qb[6] = SolBack[j].qv[5];
                if (smoother(SolForward[i].rr + 3, Qf, SolBack[j].rr + 3, Qb, 3, sols.rr + 3, Qs))
                    continue;
...
static int execses(gtime_t ts, gtime_t te, double ti, const prcopt_t *popt, const solopt_t *sopt, const filopt_t *fopt,
    ...
    /* read dcb parameters */
    if (*fopt->osb) {
        reppath(fopt->osb, path, ts, "", "");
        readosb(path, &Navs, Stas);
    }
    /* set antenna paramters */
    if (popt_.mode != PMODE_SINGLE) {
        setpcv(Obss.n > 0 ? Obss.data[0].time : timeget(), &popt_, &Navs, &PcvSat, &PcvRcv, Stas);
    }
    /* read ocean tide loading parameters */
    ...
    IobsRover = IobsBase = Revs = Aborts = 0;
    if (popt_.mode == PMODE_SINGLE || popt_.soltype == 0) {
        ...
        if ((fp = openfile(outfile))) {
            Revs  = 1;
            IobsRover = IobsBase = Obss.n - 1;
            procpos(fp, &popt_, sopt, 0); /* backward */
            fclose(fp);
        }
    } else { /* combined */
        SolForward = (sol_t *) malloc(sizeof(sol_t) * Nepoch);
        SolBack = (sol_t *) malloc(sizeof(sol_t) * Nepoch);
        RbaseForward  = (double *) malloc(sizeof(double) * Nepoch * 3);
        RbaseBack  = (double *) malloc(sizeof(double) * Nepoch * 3);

        if (SolForward && SolBack) {
            IsolForward = IsolBack = 0;
