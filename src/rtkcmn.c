...
const prcopt_t PrcoptDefault        = {
    /* defaults processing options */
    PMODE_PPP_KINEMA, 0, 2, SYS_GPS,    /* mode,soltype,nf,navsys */
    15.0 * D2R, {{0, 0},{{0}}},         /* elmin,snrmask */
    EPHOPT_PREC, 1, 1, 1, 1,1,            /* sateph,modear,glomodear,galmodear,bdsmodear, methodar */
    5, 0, 10, 1,                        /* maxout,minlock,minfix,armaxiter */
    0, 0, 0, 0,                         /* estion,esttrop,dynamics,tidecorr */
    1, 0, 0,                            /* niter,codesmooth,intpref */
    0, 0,                               /* rovpos,refpos */
    ...
    0.0, 0.0, {0.05,10.0},              /* elmaskar,almaskhold,thresslip */
    30.0, 50.0, 30.0,                   /* maxtdif,maxinno,maxgdop */
    {0}, {0}, {0},                      /* baseline,ru,rb */
    {"", ""},                           /* anttype */
    {{0}}, {{0}}, {0},                  /* antdel,pcv,exsats */
    0,                                  /*outsingle*/
    {"",""},                            /* rinex opt rover/base */
    {1,1,1,1,1,0},                      /* sat pcv, rcv pcv, phase windup, test eclipse, RAIM FDE, handle day-boundary clock jump*/
    {0},                                /* ocean-disp */
    120.0,                              /* maxgapiono */
};
const solopt_t SoloptDefault = {
    /* defaults solution output options */
    SOLF_LLH, TSYS_UTC     , 1, 3,          /* posf,times,timef,timeu */
...
// keep order: GREJSCIL
const char SysCodes[NSYS+1]    = "GREJSCIL";
const int NavSyss[NSYS+1] = {SYS_GPS, SYS_GLO, SYS_GAL, SYS_QZS, SYS_SBS, SYS_CMP, SYS_IRN,SYS_LEO,SYS_NONE};
// must keep one-to-one match: freqpris freqpris CodePris
static const char FreqCodes[NSYS][MAXFREQ]  = {{'1', '2', '5'},
                                               {'1', '2', '3', '4', '6'},
                                               {'1', '5', '6', '7', '8'},
                                               {'1', '2', '5', '6'},
                                               {'1', '5'},
                                               {'2', '6', '7', '1', '5', '8'},
                                               {'5', '9'},
                                               {'1', '5'}}; // warn: LEO frequency
const double FreqVals[NSYS][MAXFREQ] = {{FREQ1, FREQ2, FREQ5},
                                               {FREQ1_GLO, FREQ2_GLO, FREQ3_GLO, FREQ1a_GLO, FREQ2a_GLO},
                                               {FREQ1, FREQ5, FREQ6, FREQ7, FREQ8},
                                               {FREQ1, FREQ2, FREQ5, FREQ6},
                                               {FREQ1, FREQ5},
                                               {FREQ1_CMP, FREQ3_CMP, FREQ2_CMP, FREQ1, FREQ5, FREQ8},
                                               {FREQ5, FREQ9},
                                               {FREQ1, FREQ5}}; // warn: LEO frequency
static char CodePris[NSYS][MAXFREQ][16]     = {
    /* code priority for each freq-index */
    /*    0         1          2          3         4         5     */
    {"CPYWMNSL", "PYWCMNDLSX", "IQX", "", "", "", ""},       /* GPS 125 */
    {"CPABX", "PCABX", "IQX", "", "", "", ""},               /* GLO 14 26 3*/
    {"CABXZ", "IQX", "ABCXZ", "IQX", "IQX", "", ""},         /* GAL 15678 */
    {"CLSXZ", "LSX", "IQXDPZ", "LSXEZ", "", "", ""},         /* QZS 1256 */
    {"C", "IQX", "", "", "", "", ""},                        /* SBS 15 */
    {"IQX", "IQXDPZ", "IQXDPZ", "DPXSLZ", "DPX", "DPX", ""}, /* BDS 267158 */
    {"ABCX", "ABCX", "", "", "", "", ""},                    /* IRN 59 */
    {"IQX", "IQX", "", "", "", "", ""}                       /* LEO */  // warn: LEO frequency code
};
...
// system index in NavSyss, return -1 if not in NavSyss
extern int sys2idx(int sys) {
    int isys = -1;
    for (isys = 0; isys < NSYS; ++isys) {
        if(NavSyss[isys] == sys){return isys;}
    }
    return -1;
}
 ...
 *-----------------------------------------------------------------------------*/
extern int satno(int sys, int prn) { // todo resort satno by system index
    if (prn <= 0)
        return 0;
    switch (sys) {
        case SYS_GPS:
...
// return 2: bds2, 3: bds3, other: not bds
extern int bds2or3(int sat) {
    const int c01 = satid2no("C01");
    if (sat >= c01 && sat <= c01 + 17) {
        return 2;
    }
    if (sat >= c01 + 18 && sat < c01 + NSATCMP) {
        return 3;
    }
    return 0;
}

extern int sat2orbtype(int sat) {
    const int c01 = satid2no("C01");
    if(satsys(sat,NULL)== SYS_LEO){
        return ORB_LEO;
    }
    if ((sat >= c01 && sat <= c01 + 4) || (sat >= c01 + 58 && sat <= c01 + 60)) {
        return ORB_GEO;
    }
    if ((sat >= c01 + 5 && sat <= c01 + 9) || sat == c01 + 12 || sat == c01 + 15 || sat == c01 + 30 ||
        (sat >= c01 + 37 && sat <= c01 + 39) || sat == c01 + 55) {
        return ORB_IGSO;
    }
    return ORB_MEO;
}
...
extern void satno2id(int sat, char *id) {
    int prn;
    switch (satsys(sat, &prn)) {// warn: syscode
        case SYS_GPS:
...
// frequency index in FreqCodes, -1 if unsupport 
extern int cfreq2idx(int sys, const char *cfreq, double *freq) {
    int isys = -1, ifreq = -1;
    if(cfreq==NULL){
        return -1;
    }
    for (isys = 0; isys < NSYS; ++isys) {
        if (NavSyss[isys] == sys) {
            for (ifreq = 0; ifreq < MAXFREQ; ++ifreq) {
                if (FreqCodes[isys][ifreq] == 0) {
                    break;
                }
                if (FreqCodes[isys][ifreq] == *cfreq) {
                    if (freq)
                        *freq = FreqVals[isys][ifreq];
                    return ifreq;
                }
            }
        }
...
// return FreqVals[isys][ifreq],0 if unsupport
extern double idx2freq(int sys,int ifreq){
    int isys;

    isys = sys2idx(sys);
    if (isys < 0 || isys >= NSYS || ifreq < 0 || ifreq >= strlen(FreqCodes[isys])) {
        return 0;
    }

    return FreqVals[isys][ifreq];
}
 ...
 *            QZSS      L1    L2    L5    L6     -
 *            SBAS      L1     -    L5     -     -
 *           xBDS       B1    B2    B2a   B3   B2ab (B1=B1I,B1C,B2=B2I,B2b)
 *            BDS       B1    B3    B2(b) B1C  B2a  B2  // 2022.9.21 change
 *            NavIC     L5     S     -     -     -
 *-----------------------------------------------------------------------------*/
extern int code2idx(int sys, uint8_t code, double *freq) {
    char *obs = code2obs(code);
    return cfreq2idx(sys, obs, freq);
}
...
extern int getcodepri(int sys, uint8_t code, const char *opt) {
    ...
    switch (sys) { // warn: sysindex syscode
        case SYS_GPS:
            i      = 0;
            optstr = "-GL%2s";
            break;
...
extern int filter(double *x, double *P, const double *H, const double *v, const double *R, int n, int m) {
    ...
    ix = imat(n, 1);
    for (i = k = 0; i < n; i++) //!! warn: parameters that have not been initialized will not be estimated
...
static int readantex(const char *file, pcvs_t *pcvs) {
    ...
    pcv_t pcv;
    double neu[3];
    int i, f, freq = 0, state = 0, sys = SYS_NONE, npcv = 0;
    char buff[1024];
    ...
    while (fgets(buff, sizeof(buff), fp)) {
        ...
        if (strstr(buff + 60, "TYPE / SERIAL NO")) {
            ...
            if (freq < 1 || NFREQ < freq)
                continue;
            npcv = (pcv.zen2 - pcv.zen1) / pcv.dzen + 1;
            if (npcv > MAXNPCV) {
                trace(2, "warning: [readantex] too many pcv in one line, will be truncated %d->%d ", npcv, MAXNPCV);
                npcv     = MAXNPCV;
                pcv.zen2 = pcv.zen1 + (npcv - 1) * pcv.dzen; // reset zen2 
            }
            if ((i = decodef(buff + 8, npcv, pcv.var[freq - 1])) <= 0)
                continue;
            if (i < npcv) {
                trace(2, "warning: [readantex] lack of pcv, will be filled with the last %d/%d ", i, npcv);
            }
            for (; i < npcv; i++)
                pcv.var[freq - 1][i] = pcv.var[freq - 1][i - 1];
        } else {
            // todo add azi-dependent pcv
        }
    }
    fclose(fp);
...
extern int readpcv(const char *file, pcvs_t *pcvs) {
    ...
    for (i = 0; i < pcvs->n; i++) {
        ...
        for (j = 2; j < NFREQ; j++) { /* L3,L4,... */
            ...
            matcpy(pcv->var[j], pcv->var[1], MAXNPCV, 1);
        }
    }
    return stat;
}
...
static void uniqeph(nav_t *nav) {
    ...
    for (i = 1, j = 0; i < nav->n; i++) { // 2022.12.03 add compare time and iodc
        if (nav->eph[i].sat != nav->eph[j].sat || nav->eph[i].toc.time != nav->eph[j].toc.time ||
            nav->eph[i].iode != nav->eph[j].iode || nav->eph[i].iodc != nav->eph[j].iodc) {
            nav->eph[++j] = nav->eph[i];
        }
    }
    nav->n = j + 1;
...
static void uniqgeph(nav_t *nav) {
    ...
    for (i = j = 0; i < nav->ng; i++) {
        if (nav->geph[i].sat != nav->geph[j].sat || nav->geph[i].toe.time != nav->geph[j].toe.time ||
            nav->geph[i].svh != nav->geph[j].svh) { // todo add iode ? 
...
static FILE *FpTrace = NULL;     /* file pointer of trace */
static char FileTrace[1024];     /* trace file */
static int LevelTrace     = 0;   /* level of trace */
static uint32_t TickTrace = 0;   /* tick time at traceopen (ms) */
static gtime_t TimeTrace  = {0}; /* time at traceopen */
static lock_t LockTrace;         /* lock for trace */
...
static void traceswap(void) {
    ...
    if ((int) (time2gpst(time, NULL) / INT_SWAP_TRAC) == (int) (time2gpst(TimeTrace, NULL) / INT_SWAP_TRAC)) {
        unlock(&LockTrace);
        return;
    }
    TimeTrace = time;

    if (!reppath(FileTrace, path, time, "", "")) {
        unlock(&LockTrace);
        return;
    }
    if (FpTrace)
...
extern void traceopen(const char *file) {
    ...
    if (!*path || !(FpTrace = fopen(path, "w")))
        FpTrace = stderr;
    strcpy(FileTrace, file);
    TickTrace = tickget();
    TimeTrace = time;
    initlock(&LockTrace);
}
extern void traceclose(void) {
    if (FpTrace && FpTrace != stderr)
        fclose(FpTrace);
    FpTrace      = NULL;
    FileTrace[0] = '\0';
}
...
extern void trace(int level, const char *format, ...) {
    ...
    if (!FpTrace || level > LevelTrace)
        return;
    traceswap();
    fprintf(FpTrace, "%d ", level);
    va_start(ap, format);
    vfprintf(FpTrace, format, ap);
    va_end(ap);
    fflush(FpTrace);
}
extern void tracet(int level, const char *format, ...) {
    ...
    if (!FpTrace || level > LevelTrace)
        return;
    traceswap();
    fprintf(FpTrace, "%d %9.3f: ", level, (tickget() - TickTrace) / 1000.0);
    va_start(ap, format);
    vfprintf(FpTrace, format, ap);
    va_end(ap);
    fflush(FpTrace);
...
extern void traceobs(int level, const obsd_t *obs, int n) {
    ...
    for (i = 0; i < n; i++) {
        ...
        fprintf(FpTrace, " (%2d) %s %-3s rcv%d %13.3f %13.3f %13.3f %13.3f %d %d %d %d %3.1f %3.1f\n", i + 1, str, id,
...
extern void tracepeph(int level, const nav_t *nav) {
    ...
    for (i = 0; i < nav->ne; i++) {
        ...
        for (j = 0; j < MAXSAT; j++) {
            satno2id(j + 1, id);
            fprintf(FpTrace, "%-3s %d %-3s %13.3f %13.3f %13.3f %13.3f %6.3f %6.3f %6.3f %6.3f\n", s,
...
static double interpvar(double ang, double zen1, double zen2, double dzen, const double *var) {
    int i, nzen;
    double a;

    if (dzen == 0 || (zen1 == 0 && zen2 == 0)) { // empty table
        return 0;
    }
    if (ang < zen1 || ang > zen2) {
        // trace(2, "warning: out of pcv table %lf (%lf,%lf)/%lf \n", ang, zen1, zen2, dzen);
    }
    ...
    i = (ang - zen1) / dzen;
    a = (ang - (zen1 + i * dzen)) / dzen;
    return var[i] * (1.0 - a) + var[i + 1] * a;
}
...
extern void antmodel(const pcv_t *pcv, const double *del, const double *azel, int opt, double *dant) {
    ...
    for (i = 0; i < NFREQ; i++) {
        ...
        dant[i] = -dot(off, e, 3) +
                  (opt ? interpvar(90.0 - azel[1] * R2D, pcv->zen1, pcv->zen2, pcv->dzen, pcv->var[i]) : 0.0);
    }
...
extern void antmodel_s(const pcv_t *pcv, double nadir, double *dant) {
    ...
    for (i = 0; i < NFREQ; i++) {
        dant[i] = interpvar(nadir * R2D, pcv->zen1, pcv->zen2, pcv->dzen, pcv->var[i]);
    }
