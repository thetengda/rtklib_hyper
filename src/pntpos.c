...
static double prange(const obsd_t *obs, const nav_t *nav, const prcopt_t *opt, double *var) {
    ...
    *var = 0.0;
    ...
    P1 -= nav->satosb[sat - 1][0][obs->code[0] - 1];
    if(P2 != 0.0)
        P2 -= nav->satosb[sat - 1][0][obs->code[1] - 1];

    // todo tgd correction for GAL/BDS
...
static int rescode(int iter, const obsd_t *obs, int n, const double *rs, const double *dts, const double *vare,
    ...
    int i, j, nv = 0, sat, sys, mask[NX - 3] = {0}, isys=-1;
...
static int valsol(const double *azel, const int *vsat, int n, const prcopt_t *opt, const double *v, int nv, int nx,
    ...
    if (nv > nx && vv > ChiSqr[nv - nx - 1]) {
        sprintf(msg, "chi-square warning nv=%d vv=%.1f cs=%.1f", nv, vv, ChiSqr[nv - nx - 1]);
        // return 0;
    }
    /* large GDOP check */
    for (i = ns = 0; i < n; i++) {
    ...
    if (dop[0] <= 0.0 || dop[0] > opt->maxgdop) {
        sprintf(msg, "gdop warning nv=%d gdop=%.1f", nv, dop[0]);
        // return 0;
    }
    return 1;
}
...
static int estpos(const obsd_t *obs, int n, const double *rs, const double *dts, const double *vare, const int *svh,
    ...
    for (i = 0; i < MAXITR; i++) {
        ...
        if (norm(dx, NX) < 1E-4) {
            ...
            for(j=0;j<NSYS+1;++j) // fixed now is dtr(G R E J S C I L)
                sol->dtr[j] = x[j+3] / CLIGHT;
            for (j = 0; j < 6; j++)
