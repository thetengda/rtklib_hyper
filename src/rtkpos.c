...
static FILE *FpStat        = NULL; /* rtk status file pointer */
static char FileStat[1024] = "";   /* rtk status file original path */
static gtime_t TimeStat    = {0};  /* rtk status file time */
...
extern int rtkopenstat(const char *file, int level) {
    ...
    if (!(FpStat = fopen(path, "w"))) {
        trace(1, "rtkopenstat: file open error path=%s\n", path);
        return 0;
    }
    strcpy(FileStat, file);
    TimeStat = time;
    StatLevel = level;
    return 1;
}
...
static void swapsolstat(void) {
    ...
    if ((int) (time2gpst(time, NULL) / INT_SWAP_STAT) == (int) (time2gpst(TimeStat, NULL) / INT_SWAP_STAT)) {
        return;
    }
    TimeStat = time;

    if (!reppath(FileStat, path, time, "", "")) {
        return;
    }
    if (FpStat)
        fclose(FpStat);
...
static void outsolstat(rtk_t *rtk) {
    ...
    if (StatLevel <= 0 || !FpStat || !rtk->sol.stat)
        return;
    ...
    for (i = 0; i < MAXSAT; i++) {
        ...
        satno2id(i + 1, id);
        for (j = 0; j < nfreq; j++) {
            fprintf(FpStat, "$SAT,%d,%.3f,%s,%d,%.1f,%.1f,%.4f,%.4f,%d,%.1f,%d,%d,%d,%d,%d,%d\n", week, tow, id, j + 1,
...
static void udpos(rtk_t *rtk, double tt) {
    ...
    Q[0] = Q[4] = SQR(rtk->opt.prn[1]) * fabs(tt);
    Q[8]        = SQR(rtk->opt.prn[2]) * fabs(tt);
    ecef2pos(rtk->x, pos);
    covecef(pos, Q, Qv);
    for (i = 0; i < 3; i++)
        for (j = 0; j < 3; j++) {
...
static void udion(rtk_t *rtk, double tt, double bl, const int *sat, int ns) {
    ...
    for (i = 1; i <= MAXSAT; i++) {
        j = II(i, &rtk->opt);
        if (rtk->x[j] != 0.0 && rtk->ssat[i - 1].outc[0] > rtk->opt.maxgapiono && rtk->ssat[i - 1].outc[1] > rtk->opt.maxgapiono)
            rtk->x[j] = 0.0;
    }
    for (i = 0; i < ns; i++) {
        j = II(sat[i], &rtk->opt);
        ...
        if (rtk->x[j] == 0.0) {
            initx(rtk, 1E-6, SQR(rtk->opt.std[6] * bl / 1E4), j);
        } else {
            /* elevation dependent factor of process noise */
            el   = rtk->ssat[sat[i] - 1].azel[1];
            fact = cos(el);
            rtk->P[j + j * rtk->nx] += SQR(rtk->opt.prn[6] * bl / 1E4 * fact) * fabs(tt);
        }
    }
}
...
static void udtrop(rtk_t *rtk, double tt, double bl) {
    ...
    for (i = 0; i < 2; i++) {
        ...
        if (rtk->x[j] == 0.0) {
            initx(rtk, INIT_ZWD, SQR(rtk->opt.std[4]), j); /* initial zwd */
            ...
            rtk->P[j + j * rtk->nx] += SQR(rtk->opt.prn[4]) * fabs(tt);

            if (rtk->opt.tropopt >= TROPOPT_ESTG) {
                for (k = 0; k < 2; k++) {
                    rtk->P[++j * (1 + rtk->nx)] += SQR(rtk->opt.prn[5]) * fabs(tt);
                }
            }
        }
    }
...
static void udbias(rtk_t *rtk, double tt, const obsd_t *obs, const int *sat, const int *iu, const int *ir, int ns,
    ...
    for (k = 0; k < nf; k++) {
        ...
        for (i = 0; i < ns; i++) {
            ...
            rtk->P[j + j * rtk->nx] += SQR(rtk->opt.prn[8]) * fabs(tt);
        ...
        for (i = 0; i < ns; i++) {
            if (bias[i] == 0.0 || rtk->x[IB(sat[i], k, &rtk->opt)] != 0.0)
                continue;
            initx(rtk, bias[i], SQR(rtk->opt.std[8]), IB(sat[i], k, &rtk->opt));
        }
        free(bias);
    }
}
...
static int zdres(int base, const obsd_t *obs, int n, const double *rs, const double *dts, const double *var,
    ...
    if (opt->tidecorr) {
        tidedisp(gpst2utc(obs[0].time), rr_, opt->tidecorr == 1 ? 1 : 7, &nav->erp, opt->odisp[base], disp);
        for (i = 0; i < 3; i++)
            rr_[i] += disp[i];
    }
    ecef2pos(rr_, pos);
...
static int ddres(rtk_t *rtk, const nav_t *nav, double dt, const double *x, const double *P, const int *sat, double *y,
                 double *e, double *azel, double *freq, const int *iu, const int *ir, int ns, double *v, double *H,
                 double *R, int *vflg) {
