...
static const char ObsTypes[] = "CLDS"; /* observation type codes */
static const double UraEph[]     = {/* RAa values (ref [3] 20.3.3.3.1.1) */
...
static void decode_obsh(FILE *fp, char *buff, double ver, int *tsys, char tobs[][MAXOBSTYPE][4], nav_t *nav,
                        sta_t *sta) {
    ...
    const char *defcodes[] = {
        ...
        "XIXIIX ", /* BDS: L125678_ */
        "  A   A", /* IRN: L__5___9 */
        "       "  /* LEO: L_______ */ // warn: LEO obscodes
    };
    double del[3];
...
static void set_index(double ver, int sys, const char *opt, char tobs[MAXOBSTYPE][4], sigind_t *ind) {
    ...
    for (i = n = 0; *tobs[i]; i++, n++) {
        ...
        ind->type[i] = (p = strchr(ObsTypes, tobs[i][0])) ? (int) (p - ObsTypes) : 0;
        ind->idx[i]  = code2idx(sys, ind->code[i], NULL);
        ind->pri[i]  = getcodepri(sys, ind->code[i], opt);
        ind->pos[i]  = -1;
    }
    /* parse phase shift options */
    
    switch (sys) {// warn: syscode sysindex
        case SYS_GPS:
            optstr = "-GL%2s=%lf";
            break;
        case SYS_GLO:
...
static int readrnxobsb(FILE *fp, const char *opt, double ver, int *tsys, char tobs[][MAXOBSTYPE][4], int *flag,
    ...
    gtime_t time           = {0};
    sigind_t index[NSYS] = {{0}};
    char buff[MAXRNXLEN];
    int i = 0, n = 0, nsat = 0, nsys = NSYS, sats[MAXOBS] = {0}, mask;

    /* set system mask */
    mask = set_sysmask(opt);

    /* set signal index */ 
    if (nsys >= 1) // warn: sysindex
    ...
    if (nsys >= 8)
        set_index(ver, SYS_LEO, opt, tobs[7], index + 7);
