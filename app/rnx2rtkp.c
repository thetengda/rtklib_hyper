...
{
    prcopt_t prcopt=PrcoptDefault;
    solopt_t solopt=SoloptDefault;
    filopt_t filopt={""};
    gtime_t ts={0},te={0};
    ...
    sprintf(filopt.solstat,"%s.stat",PROGNAME);
    
    /* load options from configuration file */
    for (i=1;i<argc;i++) {
        if (!strcmp(argv[i],"-k")&&i+1<argc) {
            resetsysopts();
            if (!loadopts(argv[++i],OptionsTable)) return -1;
    ...
    for (i=1,n=0;i<argc;i++) {
        if      (!strcmp(argv[i],"-o")&&i+1<argc) outfile=argv[++i];
        ...
        else if (!strcmp(argv[i],"-u")) solopt.times=TSYS_UTC;
            ...
            if(strcasecmp(strrchr(argv[i],'.'),".BIA") == 0){
                strcpy(filopt.osb,argv[i]);
            }else if(strcasecmp(strrchr(argv[i],'.'),".ERP") == 0){
                strcpy(filopt.eop,argv[i]);
            } else {
                infile[n++] = argv[i];
            }
        }
